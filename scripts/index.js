...
var GAME_STATE_SHOW_WINNER = 5;
  ...
  function updateSlimeVelocitiesWithKeys(s,left,right,up,down,is2) {
    // update velocities
    if(keysDown[left]) {
      if(keysDown[right]) {
        s.velocityX = 0;
    ...
    if(s.y != 0 && keysDown[down] && s.velocityY <= 7){
      if (is2){
        if(!wasPressed2[down]){
          s.velocityY = -50
        }
    ...
    if (!keysDown[down]){
      if(is2){
        wasPressed2[down] = false;
      } else {
        wasPressed1[down] = false;
      }
    }
    if (!keysDown[up]){
      if(is2){
        wasPressed2[up] = false;
      } else {
        wasPressed1[up] = false;
      }
    }
  }
  function updateSlimeVelocitiesWithDoubleKeys(s,left1,left2,right1,right2,up1,up2,down1,down2) {
    // update velocities
    ...
    if(s.y == 0 && (keysDown[up1] || keysDown[up2])) {
      if(!wasPressedSolo[up1]){
        s.velocityY = 35;
      }
      wasPressedSolo[up1] = true;
    }
    if(s.y != 0 && (keysDown[down1] || keysDown[down2]) && s.velocityY <= 7) {
      if(!wasPressedSolo[down1]){
        s.velocityY = -50;
      }
      wasPressedSolo[down1] = true;
    }
    if(!keysDown[up1] && !keysDown[up2]){
      wasPressedSolo[up1] = false;
    } 
    if(!keysDown[down1] && !keysDown[down2]){
      wasPressedSolo[down1] = false;
    }
  }
  ...
  function updateFrame() {
    if(onePlayer) {
      ...
      updateSlimeVelocitiesWithKeys(slimeLeft, KEY_A,KEY_D,KEY_W,KEY_S,false);
      updateSlimeVelocitiesWithKeys(slimeRight, KEY_LEFT,KEY_RIGHT,KEY_UP,KEY_DOWN,true);
    }
